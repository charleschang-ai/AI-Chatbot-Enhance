...
class AIAgentSource(models.Model):
    ...
    status = fields.Selection(string="Status",
                              selection=[('processing', 'Processing'), ('indexed', 'Indexed'), ('failed', 'Failed')],
                              default='processing')
    ...
    def create(self, vals_list):
        ...
        if trigger_embeddings_cron:
            print()
            self.env.ref('ai_chatbot_enhance.ir_cron_generate_ai_embedding')._trigger()
        if trigger_scrape_urls_cron:
            self.env.ref('ai_chatbot_enhance.ir_cron_ai_process_sources')._trigger()
    ...
    # def action_open_sources_dialog(self):
    #     """
    #     Open the add sources wizard.
    #     """
    #     agent_id = self.env.context.get('default_agent_id') or self.env.context.get('agent_id')
    #
    #     return {
    #         'name': _('Add Ai Source'),
    #         'type': 'ir.actions.act_window',
    #         'res_model': 'ai.agent.source.wizard',
    #         'view_mode': 'form',
    #         'target': 'new',
    #         'context': {
    ...
    def _update_ai_source_status(self, embedding_model):
        """
        ...
        """
        self.ensure_one()
        embedding_info = self._get_ai_source_embeddings_status(embedding_model)
    ...
    def _get_ai_source_embeddings_status(self, embedding_model):
        """
    ...
    def action_retry_failed_source(self):
        ...
        if self.status == 'failed':
            source_chunks = self.env['ai.embedding'].search([('checksum', '=', self.attachment_id.checksum), ('embedding_model', '=', self.agent_id._get_embedding_ai_model())])
            if source_chunks:
                source_chunks.unlink()

            if self.url:
                cron = 'ai_chatbot_enhance.ir_cron_ai_process_sources'
            elif self.attachment_id:
                cron = 'ai_chatbot_enhance.ir_cron_generate_ai_embedding'
            else:
                cron = False
    ...
    @api.model
    def create_from_ai_attachments(self, attachment_ids, agent_id):
        """
        ...
        embedding_model = agent._get_embedding_ai_model()
    ...
    @api.model
    def create_from_ai_binary_files(self, files_datas, agent_id):
        attachment_ids = self.env['ir.attachment'].create(files_datas).ids
        return self.create_from_ai_attachments(attachment_ids, agent_id)
    ...
    def _cron_process_sources(self):
        ...
        trigger_embeddings_cron = False
        if sources_by_url:
            trigger_embeddings_cron = self._process_ai_sources_content(sources_by_url)
    ...
    def _process_ai_sources_content(self, sources_by_url):
        """
        ...
        for url, url_sources in sources_by_url.items():
            ...
            if attachment_vals_list:
                self._create_ai_sources_attachments(sources_to_attach, attachment_vals_list)
    ...
    def _fetch_content(self, source):
        ...
        if source.type == 'url' and source.url:
            internal_domains = self._get_internal_domains() or []
    ...
    def _create_ai_sources_attachments(self, sources, attachments_vals):
        """
    ...
    def _get_sources_indexing_state(self, sources, updated_checksum):
        ...
        indexed_sources = sources.filtered(lambda source: source.agent_id._get_embedding_ai_model() in indexed_embedding_models)
