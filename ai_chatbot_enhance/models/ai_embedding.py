...
class AIEmbedding(models.Model):
    ...
    def _cron_generate_embedding(self, batch_size=100):
        ...
        for model, embeddings in embeddings_by_model.items():
            ...
            for batch_idx, batch in enumerate(batches):
                try:
                    ...
                    response = llm_service.get_ai_embedding(
                        input=batch_content,
        ...
        for source in staged_sources:
            embedding_model = source.agent_id._get_embedding_ai_model()
            source._update_ai_source_status(embedding_model)
    ...
    def _create_embedding_chunks(self):
        ...
        if sources_to_process:
            ...
            existing_checksum_model_pairs = [(checksum, embedding_model) for checksum, embedding_model in
                                             existing_embeddings_grouped]
            for source in sources_to_process:
                ...
                if not embedding_model:
                    source.write({
                        ...
                        'is_active': False,
                        'error_details': _(
                            "Only OpenAI and Google are supported. Invalid provider: %s") % source.agent_id.llm_model,
                    })
                    continue
                if (source.attachment_id.checksum, embedding_model) not in existing_checksum_model_pairs:
                    ...
                    content = source.attachment_id._get_ai_attachment_content()
                    if not content:
                        source.write({
                            'status': 'failed',
                        ...
                        continue
                    source.attachment_id._setup_ai_attachment_chunks(embedding_model, content)
    ...
    @api.model
    def _get_ai_similar_chunks(self, query_embedding, sources, embedding_model, top_n=5):
