...
export const aiNaturalLanguageService = {
    ...
    start(env, { bus_service, action: actionService, menu: menuService, dialog: dialogService }) {
        ...
        for (const [viewName, view] of viewRegistry.getEntries()) {
            if (view.Controller && view.Controller.props) {
                // 如果控制器定义了 props，但没有 ai，我们强行给它加上许可
                if (!view.Controller.props.ai) {
                    view.Controller.props.ai = { type: Object, optional: true };
                }
            }
        }
